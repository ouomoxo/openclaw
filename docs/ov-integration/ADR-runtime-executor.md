...
## ADR-3. Command/Verification — env-replacing argv spawn (R4 정정)

**Context.** 검증 명령을 shell injection 없이 + **sanitized env만으로** 실행해야 한다(§R4-2).

**R4 정정.** `runPluginCommandWithTimeout`을 재사용하려 했으나, 내부 core `runCommandWithTimeout`의 env가
`params.baseEnv ?? process.env`로 **host env를 병합**한다(`src/process/exec.ts:314`). SDK 래퍼는 `baseEnv`를 안 넘기므로
child env = merge(process.env, our env) → **host secret이 verification child로 누출**. env 최소화 요구와 충돌.

**Decision.** RuntimeCommandRunner는 **env를 교체**하는 최소 argv spawn(`node:child_process`,
`extensions/agent-runtime/src/process-runner.ts`) 위에 둔다. spawn `{ env: sanitizedEnv }`는 process.env와 병합하지 않는다.
maxOutputBytes 절단·AbortSignal·exit/signal 구분·timeout(SIGKILL) 직접 제공. ProcessRunner 주입(테스트는 FakeProcessRunner).

**Why.** env 격리는 hard 보안 요구이고 SDK 래퍼는 이를 만족 못 함. 범용 러너 중복 구현이 아니라 env 격리 불변식 강제(worktree
GitRunner와 동일 패턴). core `runCommandWithTimeout`을 `baseEnv:{}`로 쓰면 안전하나 plugin-reachable 아님(core 패치 회피).

**Consequences.** `exec("문자열 보간")` 없음. command policy = deny-list + executable allowlist + verification은 RuntimeRunInput ∩
profile(argv 정확 일치). high-risk(node -e/python -c/npx)는 명시 allowlist만.

### ADR-3 부록 — ACP 명령 우회 분류 (§R4-3) = **C**

조사(`extensions/codex/src/app-server/{transport-stdio,sandbox-guard,native-execution-policy}.ts`, `sandbox-exec-server/processes.ts`):
ACP/Codex app-server는 자체 shell/tool 실행(`command/exec`, `process/spawn`, `thread/shellCommand`)을 가지며, non-sandboxed 세션에서
이 native 실행은 host에서 직접 일어나 **우리 RuntimeCommandRunner를 거치지 않는다**. 우리 command policy는 **우리가 실행하는 명령
(verification)에만** 적용되고 ACP tool calls에는 적용되지 않는다. → **분류 C: ACP가 독립적으로 임의 명령을 실행할 수 있다.**
결론: command allowlist를 완전한 보안 통제로 표현하지 않는다. fixture 전용. **production blocker로 기록**(ADR-4 gate). **core patch 안 함.**
